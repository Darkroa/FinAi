      ...
      <div className="grid grid-cols-3 gap-4">
                ...
                <tr key={a.symbol} className="border-b border-[#2b3139]/50 hover:bg-[#1e2329] transition">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#f0b90b]/10 flex items-center justify-center text-sm font-bold text-[#f0b90b]">{a.icon}</div>
                      <div>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <button className="text-xs px-2.5 py-1 rounded-lg bg-[#0ecb81]/10 text-[#0ecb81] hover:bg-[#0ecb81]/20 transition flex items-center gap-1">
                        <ArrowDownLeft size={10} /> Deposit
                      </button>
                      <button className="text-xs px-2.5 py-1 rounded-lg bg-[#f6465d]/10 text-[#f6465d] hover:bg-[#f6465d]/20 transition flex items-center gap-1">
        ...
        <div className="bg-[#161a1e] border border-[#2b3139] rounded-xl p-4">
          <div className="grid grid-cols-2 gap-1 mb-4 bg-[#0b0e11] p-1 rounded-xl">
            <button onClick={() => setTab('deposit')} className={`py-2 rounded-lg text-xs font-medium transition ${tab === 'deposit' ? 'bg-[#f0b90b] text-black' : 'text-[#848e9c]'}`}>Deposit</button>
            <button onClick={() => setTab('withdraw')} className={`py-2 rounded-lg text-xs font-medium transition ${tab === 'withdraw' ? 'bg-[#f0b90b] text-black' : 'text-[#848e9c]'}`}>Withdraw</button>
          </div>
          ...
          {tab === 'deposit' ? (
            <div className="space-y-4">
              ...
              <button onClick={() => { navigator.clipboard.writeText('TRx7Xy1Qk2mN8pLJ3vHg4wEa5rBs6cFd'); toast.success('Address copied!') }}
            ...
            <form className="space-y-3" onSubmit={e => { e.preventDefault(); toast('Withdrawal request submitted (demo)') }}>
              <div>
                ...
                <input placeholder="0.00" className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl px-3 py-2.5 text-sm font-mono text-[#eaecef] focus:outline-none focus:border-[#f0b90b] transition" />
              </div>
              <div>
                <label className="text-xs text-[#848e9c] mb-1 block">Wallet Address</label>
                <input placeholder="Your wallet address" className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl px-3 py-2.5 text-sm text-[#eaecef] focus:outline-none focus:border-[#f0b90b] transition" />
              </div>
              <button type="submit" className="w-full bg-[#f0b90b] hover:bg-[#d4a30a] text-black font-semibold py-2.5 rounded-xl text-sm transition">Submit Withdrawal</button>
            </form>
          )}
        </div>
      </div>
          ...
          <tbody>
            {txHistory.map((tx, i) => (
              <tr key={i} className="border-b border-[#2b3139]/50 hover:bg-[#1e2329] transition">
                ...
                <td className="px-4 py-3 text-right text-[#848e9c] text-xs">{tx.date}</td>
