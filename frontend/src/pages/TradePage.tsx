...
export default function TradePage() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-[#161a1e] border border-[#2b3139] rounded-xl px-3 py-2">
        <button type="button" disabled={orderLoading} onClick={() => handleQuickTrade('sell')}
          className="px-5 py-2 rounded-lg text-sm font-bold bg-[#f6465d] hover:bg-[#d93d51] text-white transition active:scale-[0.98] disabled:opacity-50 flex-shrink-0">
          Sell
        </button>
        ...
        <button type="button" disabled={orderLoading} onClick={() => handleQuickTrade('buy')}
          className="px-5 py-2 rounded-lg text-sm font-bold bg-[#0ecb81] hover:bg-[#0ab56f] text-black transition active:scale-[0.98] disabled:opacity-50 flex-shrink-0">
          Buy
        </button>
        <div className="flex-1 hidden sm:flex items-center justify-end gap-1.5 text-[10px] font-mono">
      ...
      <div className="flex items-center gap-0.5 bg-[#161a1e] border border-[#2b3139] rounded-xl px-2 py-1.5">
        {([
        ...
        <button onClick={() => { const v = !showOrderForm; setShowOrderForm(v); localStorage.setItem('finai-order-form', String(v)) }} title="Place Order"
      ...
      <div className={`grid grid-cols-1 gap-3 ${chatCollapsed ? 'lg:grid-cols-1' : 'lg:grid-cols-3'}`}>
        ...
        <div className={`${chatCollapsed ? '' : 'lg:col-span-2'}`}>
          <div className={`bg-[#161a1e] border border-[#2b3139] rounded-xl overflow-hidden flex flex-col ${chartExpanded ? 'fixed inset-0 z-[9999] rounded-none border-0' : ''}`}>
            ...
            {/* Minimal chart-card header: bid/ask + settings + collapse + fullscreen */}
            <div className="flex items-center gap-1 px-3 py-1.5 border-b border-[#2b3139] flex-shrink-0" ref={prefsRef}>
              ...
              <div className="relative">
                <button onClick={() => setShowPrefs(v => !v)}
                  className={`p-1.5 rounded-lg transition ${showPrefs ? 'text-[#eaecef] bg-[#2b3139]' : 'text-[#848e9c] hover:text-[#eaecef] hover:bg-[#2b3139]'}`}>
                  <Settings size={12} />
                </button>
                {showPrefs && (
                  <div className="absolute right-0 top-full mt-1 p-3 bg-[#1e2329] border border-[#2b3139] rounded-xl shadow-2xl z-50 w-[260px] max-h-[340px] overflow-y-auto">
                    ...
                    <div className="mt-3 pt-2 border-t border-[#2b3139]">
                      ...
                      <button onClick={() => { const v = !showEntryLines; setShowEntryLines(v); localStorage.setItem('finai-entry-lines', String(v)); setShowPrefs(false) }}
                        ...
                        {showEntryLines ? 'Hide Entry Lines' : 'Show Entry Lines'}<Target size={10} />
                      </button>
                    </div>
                    <div className="mt-3 pt-2 border-t border-[#2b3139]">
              ...
              <button onClick={() => setChartCollapsed(v => !v)}
