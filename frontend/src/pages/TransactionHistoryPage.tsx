...
function txIconBg(type: string) {
  if (type === 'deposit' || type === 'p2p_receive')
    return 'bg-[#0ecb81]/10 border-[#0ecb81]/20';
  if (type === 'withdrawal' || type === 'p2p_send')
    return 'bg-[#f6465d]/10 border-[#f6465d]/20';
...
function statusBadge(s: string) {
  if (s === 'completed' || s === 'approved')
    return (
      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#0ecb81]/10 text-[#0ecb81] inline-flex items-center gap-0.5 font-medium">
        <CheckCircle size={9} /> Approved
      </span>
    );
  if (s === 'rejected' || s === 'failed')
    return (
      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#f6465d]/10 text-[#f6465d] inline-flex items-center gap-0.5 font-medium">
        <XCircle size={9} /> Failed
      </span>
    );
  return (
    <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#f0b90b]/10 text-[#f0b90b] inline-flex items-center gap-0.5 font-medium">
...
export default function TransactionHistoryPage() {
  ...
  const totalIn = txs
    .filter((t) => isIn(t.tx_type) && t.status !== 'rejected')
    .reduce((s, t) => s + t.amount_usdt, 0);

  const totalOut = txs
    .filter((t) => !isIn(t.tx_type) && t.status !== 'rejected' && ['withdrawal', 'p2p_send'].includes(t.tx_type))
    .reduce((s, t) => s + t.amount_usdt, 0);
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg whitespace-nowrap font-medium transition ${
                filter === f ? 'bg-[#f0b90b] text-black' : 'text-[#848e9c] hover:text-[#eaecef]'
              }`}
            >
            ...
            <div key={tx.id} className="bg-[#161a1e] border border-[#2b3139] rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-2 mt-1.5">
                  <p className="text-[10px] text-[#848e9c] capitalize">
                    {tx.method?.replace(/_/g, ' ')} · {new Date(tx.created_at).toLocaleDateString()}
                  </p>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-[#848e9c] text-sm">
                    <RefreshCw size={20} className="animate-spin mx-auto mb-2 text-[#f0b90b]" />
                    Loading…
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((tx) => (
                  <tr key={tx.id} className="border-b border-[#2b3139]/50 hover:bg-[#1e2329] transition">
                    ...
                    <td className="px-5 py-3.5 text-xs text-[#848e9c] capitalize">
                    ...
                    <td className="px-5 py-3.5 text-xs text-[#848e9c] hidden md:table-cell max-w-[120px] truncate">
                      {tx.note || '—'}
                    </td>
