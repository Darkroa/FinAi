...
import {
  ...
  Settings, ShieldCheck, LogOut, Zap, Bell, Bot, X, ChevronDown
} from 'lucide-react'
...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-[#0b0e11] overflow-hidden">
      ...
      {/* ── Sidebar ── */}
      <aside className={cn(
        'flex-shrink-0 bg-[#161a1e] border-r border-[#2b3139] flex flex-col transition-all duration-200 overflow-hidden',
        ...
        <div className="p-3 border-t border-[#2b3139] flex-shrink-0">
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl bg-[#0b0e11] mb-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[#848e9c] text-[10px]">{user?.is_admin ? 'Admin' : 'Member'}</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* Top bar */}
        <header className="h-16 border-b border-[#2b3139] bg-[#161a1e] flex items-center justify-between px-4 gap-3 flex-shrink-0">

          {/* Left: hamburger + live ticker */}
          <div className="flex items-center gap-3 overflow-hidden min-w-0">
            <button
              onClick={() => setSidebarOpen(v => !v)}
              className="w-8 h-8 rounded-lg bg-[#0b0e11] hover:bg-[#2b3139] flex items-center justify-center transition flex-shrink-0"
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative" ref={notifRef}>
              ...
              >
                ...
                {unread > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[#f6465d] rounded-full text-[9px] font-bold text-white flex items-center justify-center leading-none">
                    {unread > 9 ? '9+' : unread}
                  </span>
                )}
              </button>
              ...
              {notifOpen && (
                <div className="absolute right-0 top-11 w-80 bg-[#161a1e] border border-[#2b3139] rounded-2xl shadow-2xl z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[#2b3139]">
                    ...
                    {unread > 0 && (
                      <button onClick={handleMarkAllRead} className="text-xs text-[#f0b90b] hover:underline">
                        Mark all read
                      </button>
                    )}
                  </div>
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="py-10 text-center">
                        <Bell size={22} className="text-[#2b3139] mx-auto mb-2" />
                        <p className="text-xs text-[#848e9c]">No notifications yet</p>
                      </div>
                    ) : notifications.slice(0, 20).map(n => (
                      <div
                        key={n.id}
                        className={cn(
                          'px-4 py-3 border-b border-[#2b3139]/50 hover:bg-[#1e2329] transition',
                          !n.is_read && 'bg-[#f0b90b]/5'
                        )}
                      >
                        <div className="flex items-start gap-2">
                          {!n.is_read && (
                            <div className="w-1.5 h-1.5 rounded-full bg-[#f0b90b] mt-1.5 flex-shrink-0" />
                          )}
            ...
            <div className="relative" ref={profileRef}>
              <button
                ...
                className="flex items-center gap-2 bg-[#0b0e11] hover:bg-[#2b3139] rounded-full pl-1 pr-2.5 py-1 transition"
              >
              ...
              {profileOpen && (
                <div className="absolute right-0 top-11 w-52 bg-[#161a1e] border border-[#2b3139] rounded-2xl shadow-2xl z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#2b3139]">
                    ...
                    <p className="text-[10px] text-[#848e9c] mt-0.5">{user?.is_admin ? 'Admin' : 'Member'}</p>
                  </div>
                  <div className="p-1.5 space-y-0.5">
                    <button
                      onClick={() => { navigate('/app/settings'); setProfileOpen(false) }}
                      className="w-full text-left px-3 py-2 text-xs text-[#848e9c] hover:text-[#eaecef] hover:bg-[#2b3139] rounded-xl transition"
                    >
                      Account Settings
                    </button>
                    {user?.is_admin && (
                    ...
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-3 py-2 text-xs text-[#f6465d] hover:bg-[#f6465d]/10 rounded-xl transition"
                    >
                      Sign Out
                    </button>
                  </div>
...
/* ── Live ticker bar using CoinGecko ── */
...
function LiveTickerBar() {
  ...
  const tickers = [
    {
      symbol: 'BTC/USDT',
      price: btcPrice ?? 67432,
      change: btcChange ?? 2.4,
      live: !loading && btcPrice !== null,
    },
    {
      symbol: 'ETH/USDT',
      price: ethPrice ?? 3521,
      change: ethChange ?? 1.8,
      live: !loading && ethPrice !== null,
    },
    ...STATIC_TICKERS.map(t => ({ ...t, live: false })),
  ]
